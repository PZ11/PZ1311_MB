...
p90d_prod_user_cnt <- orders_prod_prior %>%
  inner_join(orders_90d %>% select(user_id, order_id), by=c("user_id", "order_id")) %>%
  group_by(product_id) %>%
  summarise( p90d_p_user_cnt = n())


p90d_prod_reorder_user_cnt <- orders_prod_prior %>%
  inner_join(orders_90d %>% select(user_id, order_id), by=c("user_id", "order_id")) %>%
  filter(reordered == 1) %>%
  group_by(product_id) %>%
  summarise( p90d_p_reorder_user_cnt = n())

p90d_prod_user_cnt <- p90d_prod_user_cnt %>%
  left_join(p90d_prod_reorder_user_cnt, by = "product_id") %>%
  mutate ( p90d_p_reorder_user_rate = p90d_p_reorder_user_cnt / p90d_p_user_cnt ) 


# Get the unique user count 
p90d_prod_unique_user_cnt <- orders_prod_prior %>%
  inner_join(orders_90d %>% select(user_id, order_id), by=c("user_id", "order_id")) %>%
  group_by(user_id, product_id) %>%
  summarise(p_user_cnt_tmp = 1) %>%
  group_by(product_id) %>%
  summarise(p90d_p_unique_user_cnt = n())


# Get the unique reordered user count 
p90d_prod_unique_user_reordered_cnt <- orders_prod_prior %>%
  inner_join(orders_90d %>% select(user_id, order_id), by=c("user_id", "order_id")) %>%
  filter(reordered == 1) %>%
  group_by(user_id, product_id) %>%
  summarise(p_user_reorder_cnt_tmp = 1) %>%
  group_by(product_id) %>%
  summarise(p90d_p_unique_reorder_user_cnt = n())


# Combine all user features 
p90d_prod_features <- p90d_prod_user_cnt %>%
  inner_join( p90d_prod_unique_user_cnt , by = "product_id") %>%
  left_join(p90d_prod_unique_user_reordered_cnt, by = "product_id") %>%
  mutate ( p90d_p_unique_reorder_user_rate = p90d_p_unique_reorder_user_cnt / p90d_p_unique_user_cnt ) 

p90d_prod_features$p90d_p_reorder_user_cnt <- NULL
p90d_prod_features$p90d_p_unique_reorder_user_cnt <- NULL


data <- data %>%
  left_join(p90d_prod_features, by = "product_id") 


rm(orders_90d, p90d_prod_unique_user_cnt, p90d_prod_features,
   p90d_prod_unique_user_reordered_cnt, p90d_prod_user_cnt, p90d_prod_reorder_user_cnt)
...
#### Pl4o User_Product Features  ###########################################

orders_l4o <-  orders %>% 
  filter(eval_set == "prior") %>%
  group_by(user_id) %>%
  mutate( l4o_order_number = max(order_number) - 4  ) %>%
  filter(order_number > l4o_order_number) 


users_l4o <- orders_l4o %>%
  group_by(user_id) %>%
  summarise(
    pl4o_max_orders = max(order_number),
    pl4o_min_orders = min(order_number)
  ) %>%
  mutate( pl4o_user_orders = pl4o_max_orders - pl4o_min_orders + 1 ) %>%
  select(user_id, pl4o_user_orders)

data_l4o <- orders_prod_prior %>%
  inner_join(orders_l4o %>% select(user_id, order_id), by=c("user_id", "order_id")) %>%
  group_by(user_id, product_id) %>% 
  summarise(
    pl4o_up_orders = n()
  )
...
data_l4o <- data_l4o %>% 
  inner_join(users_l4o, by = "user_id")

data_l4o$pl4o_up_order_rate <- data_l4o$pl4o_up_orders / data_l4o$pl4o_user_orders

#data_l4o$pl4o_user_orders <- NULL

data <- data %>%
  left_join(data_l4o, by = c("user_id", "product_id")) %>%
  mutate( pl4o_up_order_cnt_ratio_inall = pl4o_up_orders / user_order_cnt ) %>%
  mutate( pl4o_u_order_cnt_ratio_inall = pl4o_user_orders / user_order_cnt )

data_10 <- data[data$user_id <= 10, ]

rm(data_l4o, orders_l4o, users_l4o, data_10)
gc()
...
datanew <- data.table(
  ...
  # apply 5 pl4o features
  #pl4o_up_orders = data$pl4o_up_orders,
  #pl4o_user_orders = data$pl4o_user_orders,
  #pl4o_up_order_rate = data$pl4o_up_order_rate,
  #pl4o_up_order_cnt_ratio_inall = data$pl4o_up_order_cnt_ratio_inall,
  #pl4o_u_order_cnt_ratio_inall = data$pl4o_u_order_cnt_ratio_inall , 
  ...
  #p90d_p_unique_reorder_user_rate = data$p90d_p_unique_reorder_user_rate,
  #p90d_p_reorder_user_rate = data$p90d_p_reorder_user_rate,

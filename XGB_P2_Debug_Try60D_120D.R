...
user_dept_dow_hod <- orders_prod_prior %>%
  inner_join( products %>% select(product_id, department), by = "product_id") %>%
  group_by(user_id, department) %>%
  summarise( ud_dow_mean = mean(order_dow),
             #u_dow_sd = sd(order_dow),
             ud_hod_mean = mean(order_hour_of_day)
             #u_hod_sd = sd(order_hour_of_day)                   
  )

user_dept_order_ratio <- user_dept_dow_hod %>%
  inner_join(orders %>% filter(eval_set != "prior") %>%
               select(user_id, order_dow, order_hour_of_day, days_since_prior_order), by="user_id") %>%
  mutate(ud_dow_ratio = order_dow / ud_dow_mean, 
         ud_hod_ratio = order_hour_of_day / ud_hod_mean)

user_dept_order_ratio$order_dow <- NULL
user_dept_order_ratio$order_hour_of_day <- NULL
user_dept_order_ratio$days_since_prior_order <- NULL

# Get  product count and reordered ratio 
user_dept_prodcnt <- orders_prod_prior %>%
  inner_join( products %>% select(product_id, department), by = "product_id") %>%
  group_by(user_id, department) %>%
  summarise(
    ud_prod_cnt = n(),
    ud_reorder_prod_cnt = sum(reordered,na.rm = T)
  ) %>%
  mutate(ud_reorder_prod_rate = ud_reorder_prod_cnt / ud_prod_cnt)

# Get the unique product count and reordered ratio 
user_dept_unique_prod_cnt <- orders_prod_prior %>%
  inner_join( products %>% select(product_id, department), by = "product_id") %>%
  group_by(user_id, product_id, department) %>%
  summarise(ud_prod_cnt_tmp = 1) %>%
  group_by(user_id, department) %>%
  summarise(ud_unique_prod_cnt = n())

user_dept_unique_prod_reordered_cnt <- orders_prod_prior %>%
  filter(reordered == 1) %>%
  inner_join( products %>% select(product_id, department), by = "product_id") %>%
  group_by(user_id, product_id, department) %>%
  summarise(ud_prod_cnt_tmp = 1) %>%
  group_by(user_id, department) %>%
  summarise(ud_unique_reorder_prod_cnt = n())


# Combine all user features 
user_dept <- user_dept_order_ratio %>%
  inner_join(user_dept_prodcnt, by = c("user_id", "department") ) %>%
  inner_join(user_dept_unique_prod_cnt, by = c("user_id", "department") ) %>%
  left_join(user_dept_unique_prod_reordered_cnt, by = c("user_id", "department") ) %>%
  mutate ( ud_unique_reorder_prod_rate = ud_unique_reorder_prod_cnt / ud_unique_prod_cnt ) 

user_dept$order_dow <- NULL
user_dept$order_hour_of_day <- NULL
user_dept$days_since_prior_order <- NULL

rm(user_dept_prodcnt, user_dept_dow_hod, user_dept_order_ratio, user_dept_unique_prod_cnt, user_dept_unique_prod_reordered_cnt)


# Add user Features
data <- data %>% 
  left_join( products %>% select(product_id, department), by = "product_id") %>%
  left_join(user_dept , by = c("user_id", "department") ) 
...
#### P120d User_Product Features  ###########################################

orders_p120d <-  orders %>% 
  filter(eval_set == "prior") %>%
  group_by(user_id) %>%
  mutate( p120d_order_number = max(order_number) - 4  ) %>%
  filter(order_number > p120d_order_number) 


users_p120d <- orders_p120d %>%
  group_by(user_id) %>%
  summarise(
    p120d_max_orders = max(order_number),
    p120d_min_orders = min(order_number)
  ) %>%
  mutate( p120d_user_orders = p120d_max_orders - p120d_min_orders + 1 ) %>%
  select(user_id, p120d_user_orders)

data_p120d <- orders_prod_prior %>%
  inner_join(orders_p120d %>% select(user_id, order_id), by=c("user_id", "order_id")) %>%
  group_by(user_id, product_id) %>% 
  summarise(
    p120d_up_orders = n()
  )

data_p120d <- data_p120d %>% 
  inner_join(users_p120d, by = "user_id")

data_p120d$p120d_up_order_rate <- data_p120d$p120d_up_orders / data_p120d$p120d_user_orders

#data_p120d$p120d_user_orders <- NULL

data <- data %>%
  left_join(data_p120d, by = c("user_id", "product_id")) %>%
  mutate( p120d_up_order_cnt_ratio_inall = p120d_up_orders / user_order_cnt ) %>%
  mutate( p120d_u_order_cnt_ratio_inall = p120d_user_orders / user_order_cnt )

data_10 <- data[data$user_id <= 10, ]

rm(data_p120d, orders_p120d, users_p120d, data_10)
gc()



#### p60d User_Product Features  ###########################################

orders_p60d <-  orders %>% 
  filter(eval_set == "prior") %>%
  group_by(user_id) %>%
  mutate( p60d_order_number = max(order_number) - 4  ) %>%
  filter(order_number > p60d_order_number) 


users_p60d <- orders_p60d %>%
  group_by(user_id) %>%
  summarise(
    p60d_max_orders = max(order_number),
    p60d_min_orders = min(order_number)
  ) %>%
  mutate( p60d_user_orders = p60d_max_orders - p60d_min_orders + 1 ) %>%
  select(user_id, p60d_user_orders)

data_p60d <- orders_prod_prior %>%
  inner_join(orders_p60d %>% select(user_id, order_id), by=c("user_id", "order_id")) %>%
  group_by(user_id, product_id) %>% 
  summarise(
    p60d_up_orders = n()
  )
...
data_p60d <- data_p60d %>% 
  inner_join(users_p60d, by = "user_id")

data_p60d$p60d_up_order_rate <- data_p60d$p60d_up_orders / data_p60d$p60d_user_orders

#data_p60d$p60d_user_orders <- NULL

data <- data %>%
  left_join(data_p60d, by = c("user_id", "product_id")) %>%
  mutate( p60d_up_order_cnt_ratio_inall = p60d_up_orders / user_order_cnt ) %>%
  mutate( p60d_u_order_cnt_ratio_inall = p60d_user_orders / user_order_cnt )

data_10 <- data[data$user_id <= 10, ]

rm(data_p60d, orders_p60d, users_p60d, data_10)
gc()
...
datanew <- data.table(
  ...
  # apply 5 p120d features
  p120d_up_orders = data$p120d_up_orders,
  p120d_user_orders = data$p120d_user_orders,
  p120d_up_order_rate = data$p120d_up_order_rate,
  p120d_up_order_cnt_ratio_inall = data$p120d_up_order_cnt_ratio_inall,
  p120d_u_order_cnt_ratio_inall = data$p120d_u_order_cnt_ratio_inall , 
  
  # apply 5 p60d features
  p60d_up_orders = data$p60d_up_orders,
  p60d_user_orders = data$p60d_user_orders,
  p60d_up_order_rate = data$p60d_up_order_rate,
  p60d_up_order_cnt_ratio_inall = data$p60d_up_order_cnt_ratio_inall,
  p60d_u_order_cnt_ratio_inall = data$p60d_u_order_cnt_ratio_inall , 
  ...
  #p60d_p_unique_reorder_user_rate = data$p60d_p_unique_reorder_user_rate,
  #p60d_p_reorder_user_rate = data$p60d_p_reorder_user_rate,
  
  #ud_hod_mean = data$ud_hod_mean, 
  #ud_dow_mean = data$ud_dow_mean, 
  #ud_dow_ratio = data$ud_dow_ratio, 
  #ud_hod_ratio = data$ud_hod_ratio, 
  
  #ud_prod_cnt = data$ud_prod_cnt, 
  #ud_reorder_prod_rate = data$ud_reorder_prod_rate, 
  #ud_unique_prod_cnt = data$ud_unique_prod_cnt, 
  #ud_unique_reorder_prod_rate = data$ud_unique_reorder_prod_rate, 
